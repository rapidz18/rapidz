...
// rapidz: optional setting to unlock wallet for block minting only;
...
// rapidz: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // rapidz: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // rapidz: sub-cent change is moved to fee
...
// rapidz: create coin stake transaction
...
// rapidz: check 'spent' consistency between wallet and txindex
// rapidz: fix wallet spent state according to txindex
...
// rapidz: disable transaction (only for coinstake)
